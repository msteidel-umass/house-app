...
export default defineComponent({
	...
	setup() {
		...
		function drawGraph(geo_id) {
			...
			var height = document.getElementById("graph").clientHeight - 30; // Decreased height by 30 to prevent the graph from getting cut off

			var margin = { top: 10, right: 40, bottom: 20, left: 75 }; // Decreased bottom margin to fit more space for the graph
			...
			d3.csv(HOME_VALS).then(function (data) {
				...
				var y = d3.scaleLinear()
					.domain([yMinValue - yRange * 0.05, yMaxValue + yRange * 0.05])
					.range([height, 0]);
